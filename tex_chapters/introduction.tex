...
as a study which `aims to detect the arguments presented in a text document, the relations between them and the internal structure of each individual argument'. In this context three main 
tasks were defined to address the various aspects which argumentation involves:
\begin{itemize}
    \item argument identification, where the goal is to isolate the parts which are considered argumentative from the ones which are deemed non-argumentative;
    \item argument component identification, where the objective is to identify and classify argument components as either claims, assertions of a given fact which the speaker wants to prove in the text, or premises, 
    axioms which are assumed to be true in the text;
    \item argument relation identification, where the goal is to identify and classify different types of relationship between claims and premises (i.e. a premise can be used to support a claim but also to contest it).
\end{itemize}
